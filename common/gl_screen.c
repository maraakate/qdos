...
void SCR_Init (void)
{
	...
	Cvar_SetDescription("show_fps", "Show framerate measured in Frames Per Second.");
	show_time = Cvar_Get("show_time","0", CVAR_ARCHIVE); 
	Cvar_SetDescription("show_time", "Show current time in the HUD.  1 for military.  2 for AM/PM.");
	show_uptime = Cvar_Get("show_uptime","0", CVAR_ARCHIVE);
	Cvar_SetDescription("show_uptime", "Show uptime.");
#ifdef QUAKEWORLD
