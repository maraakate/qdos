...
//					Sbar_DrawString (160-14*8+4,4, "SPECTATOR MODE - TRACK CAMERA");
				Com_sprintf(st, sizeof(st), "Tracking %-.13s, [JUMP] for next",
...
//	l = scoreboardlines;
	...
	// draw pings
		...
		Com_sprintf (num, sizeof(num), "%3i/%3i/%3i", plow, pavg, phigh);
		Draw_String ( x, y, num);
	...
	// draw total
		Com_sprintf (num, sizeof(num), "%5i", tm->frags);
		Draw_String (x + 104 + 40, y, num);
		
	// draw players
		Com_sprintf (num, sizeof(num), "%5i", tm->players);
...
//		Draw_String ( x , y, "---- -- ---- ----- ----------------");
	...
	for (i=0 ; i<l && y <= vid.height-10 ; i++)
	{
		...
		if (p < 0 || p > 999)
			p = 999;
		Com_sprintf (num, sizeof(num), "%4i", p);
		Draw_String ( x, y, num);

		// draw pl
		p = s->pl;
		Com_sprintf (num, sizeof(num), "%3i", p);
		if (p > 25)
		...
		Com_sprintf (num, sizeof(num), "%4i", minutes);
